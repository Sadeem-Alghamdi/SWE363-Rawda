...
export default function ForumModeration() {
  ...
  const [actionType, setActionType] = useState<"remove" | "warn" | "safe">("remove");
  ...
  const handleAction = (post: ForumPost, type: "remove" | "warn" | "safe") => {
    setSelectedPost(post);
    setActionType(type);
  ...
  const confirmAction = () => {
    if (selectedPost) {
      if (actionType === "remove") {
        ...
        setPosts((prev) =>
          prev.map((p) => (p.id === selectedPost.id ? { ...p, status: "Warned" as const } : p))
        );
