...
export default function BadgesSystem() {
  ...
  const confirmAssignment = (badgeName, badgeColor) => {
    if (selectedExpert) {
      setExperts((prev) =>
        prev.map((e) =>
            ...
            <tbody className="divide-y divide-gray-200">
              {experts.map((expert) => (
                <tr key={expert.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <button onClick={() => handleAssignBadge(expert)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => removeBadge(expert.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors">
      ...
      {showAssignModal && selectedExpert && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            ...
            <button
              onClick={() => { setShowAssignModal(false); setSelectedExpert(null); }}
