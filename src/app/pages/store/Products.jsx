...
export default function Products() {
  ...
  const [products, setProducts] = useState([
    { id: "1", name: "Monstera Deliciosa", category: "Indoor Plants", price: 45.99, stock: 12, status: "In Stock", description: "Beautiful tropical plant" },
    { id: "2", name: "Snake Plant", category: "Indoor Plants", price: 29.99, stock: 5, status: "Low Stock" },
    { id: "3", name: "Lavender", category: "Outdoor Plants", price: 15.99, stock: 0, status: "Out of Stock" },
    { id: "4", name: "Organic Fertilizer", category: "Supplies", price: 12.50, stock: 25, status: "In Stock" },
  ]);
  ...
  const handleEditProduct = (product) => {
    ...
    setFormData({ name: product.name, category: product.category, price: product.price.toString(), stock: product.stock.toString(), description: product.description || "" });
    setFormErrors({});
    setShowModal(true);
  };
  ...
  const handleSubmit = () => {
    ...
    if (modalMode === "add") {
      setProducts((prev) => [...prev, { id: Date.now().toString(), name: formData.name, category: formData.category, price, stock, status, description: formData.description }]);
      showToast("Product added successfully.");
    } else if (selectedProduct) {
      setProducts((prev) => prev.map((p) => p.id === selectedProduct.id ? { ...p, name: formData.name, category: formData.category, price, stock, status, description: formData.description } : p));
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={handleAddProduct} className="flex items-center gap-2 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors">
                ...
                <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleDeleteProduct(product)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2 border rounded-lg ${formErrors.price ? "border-red-500" : "border-gray-300"}`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2 border rounded-lg ${formErrors.stock ? "border-red-500" : "border-gray-300"}`}
                  />
              ...
              <div>
                ...
                {imagePreview ? (
                  <div className="relative w-full h-32 border border-gray-300 rounded-lg flex items-center justify-center">
                    ...
                    <button onClick={() => setImagePreview(null)} className="absolute top-2 right-2 p-1 bg-red-600 text-white rounded-full">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                ) : (
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleSubmit} className="flex-1 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700">
      ...
      {showDeleteModal && productToDelete && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">Delete Product</h3>
            ...
            <div className="flex gap-3">
              <button onClick={() => { setShowDeleteModal(false); setProductToDelete(null); }} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
                Cancel
              </button>
